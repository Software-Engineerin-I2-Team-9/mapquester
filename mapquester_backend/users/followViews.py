...
def follow_user(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return JsonResponse({"error": f"User with ID {follower_id} does not exist."}, status=404)

        try:
            following = User.objects.get(id=following_id)
        except User.DoesNotExist:
            return JsonResponse({"error": f"User with ID {following_id} does not exist."}, status=404)
        ...
        if created:
            return JsonResponse({"message": f"{follower.username} is now following {following.username}."}, status=201)
        else:
...
def unfollow_user(request):
    ...
    if request.method == 'POST':
        ...
        if follow_relation.exists():
            ...
            return JsonResponse({"message": f"{follower.username} has unfollowed {following.username}."}, status=200)
        else:
            return JsonResponse({"error": "No such follow relationship exists."}, status=404)
...
def get_followers(request, user_id):
    ...
    followers = user.followers.all().values('follower__id', 'follower__username', 'follower__email')
...
def get_followings(request, user_id):
    ...
    # user = get_object_or_404(User, id=user_id)
    followings = user.following.all().values('following__id', 'following__username', 'following__email')
